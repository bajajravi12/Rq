...
async def handle_choice(choice):
    if choice == '1': # SINGLE HOST INSPECTOR
        ...
        for i, f in enumerate(files, 1):
            ui.console.print(f"[bold cyan][{i}][/bold cyan] [white]{f}[/white]")
        f_idx = input("\nChoice: ").strip()
        try:
        ...
        for r in results:
            ui.console.print(f"[bold cyan]Server:[/bold cyan] [white]{r['server']}[/white] | [bold cyan]Status:[/bold cyan] [white]{r['status']}[/white] | [bold cyan]WebSocket:[/bold cyan] [white]{'Yes' if r['ws'] else 'No'}[/white]")
        ...
        if os.path.exists("results"):
            logs = sorted([f for f in os.listdir("results") if f.endswith(".txt")])
            if not logs: ui.info("No logs available.")
            else:
                for l in logs: ui.console.print(f"[cyan]»[/cyan] [white]{l}[/white]")
